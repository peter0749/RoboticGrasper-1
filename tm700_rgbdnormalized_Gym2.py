  ...
  with open(output_path, 'w') as result_fp:
      ...
      with torch.no_grad():
          for ite in range(total_n):
              ...
              pred_poses = retrive_from_feature_volume(pc_npy[reverse_lookup_index[0]]+trans_to_frame[np.newaxis], pred[0], *pred[0].shape[:-1],
                      config['hand_height'],
              ...
              pred_poses = filter_out_invalid_grasp_fast(config, pc_no_arm, [ p[1] for p in pred_poses], n_collision=2)
              '''
