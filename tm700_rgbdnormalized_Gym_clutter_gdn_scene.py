...
if __name__ == '__main__':
  ...
  from gdn.representation.euler_scene import *
  ...
  with open(output_path, 'w') as result_fp:
      ...
      input_points = 8192
      ts = None #1/240.
      test = tm700_rgbd_gym(width=720, height=720, numObjects=7, objRoot='/tmp3/peter0749/YCB_valset_urdf')
      ...
      with torch.no_grad():
          for ite in range(total_n):
              ...
              while (not grasp_success_obj.all()) and grasp_failure_obj.max()<max_tries:
                  ...
                  try:
                      pc_batch = torch.from_numpy(pc_npy - trans_to_frame.reshape(1, 3)).unsqueeze(0).cuda()
                      pred = model(pc_batch).cpu().numpy()
                  except:
                      continue
                  ...
                  for pose in pred_poses:
                      ...
                      outer_pts = crop_index(pc_npy, gripper_outer1, gripper_outer2)
                      inner_pts = crop_index(pc_npy[outer_pts], gripper_inner1, gripper_inner2)
